...
TEST_CASE("Number type check")
{
    vili::node number_validator = vili::object { { "type", "number" } };
    SECTION("Test integer type")
    {
        vili::node zero = 0;
        REQUIRE_NOTHROW(vili::validator::validate_number("", number_validator, zero));
    }
    SECTION("Test string type")
    {
        vili::node hello_world = "Hello, world!";
        REQUIRE_THROWS_AS(
            vili::validator::validate_number("", number_validator, hello_world),
    ...
    SECTION("Test boolean type")
    {
        ...
        REQUIRE_THROWS_AS(
            vili::validator::validate_number("", number_validator, bool_true),
            vili::validator::exceptions::value_type_mismatch);
    }
    SECTION("Test number type")
    {
        vili::node e = 2.71828;
        REQUIRE_NOTHROW(vili::validator::validate_number("", number_validator, e));
    }
    SECTION("Test array type")
    {
        vili::node table_of_9 = vili::array { 9, 18, 27, 36, 45, 54, 63, 72, 81, 90 };
        REQUIRE_THROWS_AS(
            vili::validator::validate_number("", number_validator, table_of_9),
            vili::validator::exceptions::value_type_mismatch);
    }
    SECTION("Test object type")
    {
        ...
        REQUIRE_THROWS_AS(
            vili::validator::validate_number("", number_validator, my_family),
            vili::validator::exceptions::value_type_mismatch);
    }
}
/*
...
TEST_CASE("multiple_of validator")
{
    SECTION("Multiple of 10")
    {
        ...
        vili::node ten = 10;
        REQUIRE_NOTHROW(vili::validator::validate_integer(multiple_of_10_validator, ten));
...
TEST_CASE("min / max validator")
{
    ...
    SECTION("Negative number")
    {
        ...
        REQUIRE_THROWS_AS(
            vili::validator::validate_integer(negative_number_validator, one_thousand),
            vili::validator::exceptions::constraint_validation_failure);
    ...
    SECTION("Number in range")
    {
        ...
        vili::node zero = 0;
        REQUIRE_NOTHROW(
            vili::validator::validate_integer(between_minus_100_and_100_validator, zero));
    }
